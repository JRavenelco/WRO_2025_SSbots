...
def main_(h):
    ...
    resetSteering()
    [leftOrRight, leftDistance, rightDistance] = findVoid(100,0)
    if leftOrRight == right:
        goForward(100,300,0)
    elif leftOrRight == left:
        pass
    turn(leftOrRight,60,87)
    resetSteering()
    
    if leftOrRight == left:
        goForward(100,((leftDistance/175)*360)+400,90*leftOrRight)
    elif leftOrRight == right:
        goForward(100,((rightDistance/175)*360)+400,90*leftOrRight)
        
    while v < 11: #number of corrections return to 11
        gyroReset(0)
        angulo = dos_puntos(100,1000,0,(leftOrRight*-1)) #INVERTIR PARA CAMBIAR LADO QUE SIGUE
        """
        print("angulo: ", angulo,"\n")
        ...
        gyroReset(angulo*(leftOrRight*-1)) #INVERTIR PARA CAMBIAR LADO QUE SIGUE
        time.sleep(0.01)
        
        forwardVoid(100,0,leftOrRight)
        if leftOrRight == right:
            goForward(100,300,0)
        elif leftOrRight == left:
            pass
        turn(leftOrRight,60,87)
        resetSteering()
        goForward(100,500,90*leftOrRight)
        v += 1
        
    gyroReset(0)
    lidarDistanceForward(100,0,1650)
    running = False
