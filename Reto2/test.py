...
def cubeDetection():
    ...
    while not vision_camera_queue.full():
        ...
        if running == False:
            break
        pass
    while vision_camera_queue.get():
        cube = vision_camera_queue.get()
            ...
            while running == True:
                ...
                if inDet is not None:
                    ...
                    if i != 0:
                        ...
                        print("max index: ", max_index, " detections: ", i)
...
def main_(h):
    ...
    """
    side = findSide()
    exitParking(side,60,65)
    cube = cubeDetection()
    print("cube detected with label: ", cube.label, " area: ", cube.area, "\n")
    if cube.label == greenbox:
        print("green box detected\n")
        turn(side,60,85)
        lidarDistanceForward(60,0,350)
        if side == left:
            turn(right,60,0)
        elif side == right:
            turn(left,60,45)
